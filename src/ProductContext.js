...
export function ProductProvider({ children }) {
    ...
    const { isLoggedIn, userLoggedIn, setIsLoggedIn, setUserLoggedIn } = useAuthValue();
    ...
    //     if (cart) {
    //         let sum = 0;
    //         cart.map((item) => (sum += item.price * item.quantity)); // Ensure total reflects quantity
    //         setTotal(sum);
    //         setItemInCart(cart.length);
    //     }
    // }, []); // Added cart as dependency
